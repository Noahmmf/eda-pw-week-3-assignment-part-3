...
console.log("4. Removed last item 11 from list, new list is:", supplyChange);
...
// 572 parts that fit inside a box of 7 slot.. the obj. is to figure out how many boxes can
// hold 572 parts. We will need the total number of parts (572).. How many parts the boxes can hold (7).
// I will need how many boxes have been filled which at the start will be 0 (because i havent created the loop yet).
//then need to create another variable for left over parts because 572 isn't divisable by 7 (creates a decimal which wouldn't
//make sense for remaining parts).

let totalParts = 572; // first define how many parts we have in total.
let boxHolds = 7; // then define how many the box each part can hold.
...
while (remainingParts >= boxHolds) {
  //total parts is more than or equal to the box holds
  remainingParts -= boxHolds; //  remaining parts subtracted by 7 for each individual box
  filledBoxes++; // set a count that will continue until total parts are converted tofilledBoxes
} //end loop
